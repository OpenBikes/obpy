...
def test_api_geojson_invalid_city():
    ...
    response = client.get_latest_geojson('gnfkdgjldjls')
    response.status_code.should.be.an(int)
...
def test_api_countries_valid_parameters():
    ...
    response = client.get_countries(provider='jcdecaux')
    response.status_code.should.be.an(int)
...
def test_api_providers_valid_parameters():
    ...
    response = client.get_providers(country='France')
    response.status_code.should.be.an(int)
    response.status_code.should.be.equal(200)
...
def test_api_cities_valid_parameters():
    ...
    response = client.get_cities(slug='toulouse', predictable=1, active=1,
...
def test_api_stations_valid_parameters():
    ...
    response = client.get_stations(slug='00003-pomme', city_slug='toulouse')
    response.status_code.should.be.an(int)
    response.status_code.should.be.equal(200)
...
def test_api_updates_valid_city():
    ...
    response = client.get_updates(city_slug='toulouse')
    response.status_code.should.be.an(int)
    response.status_code.should.be.equal(200)
...
def test_api_filtered_without_forecasts():
    ...
    response = client.get_filtered_stations(
        city_slug='toulouse',
        latitude=43.6,
        longitude=1.4333,
        limit=1
...
def test_api_closest_city():
    ...
    response = client.get_closest_city(
        latitude='43.6',
        longitude='1.4333'
    )
    response.status_code.should.be.an(int)
...
def test_api_closest_station():
    ...
    response = client.get_closest_station(
        latitude='43.6',
        longitude='1.4333'
    )
    response.status_code.should.be.an(int)
